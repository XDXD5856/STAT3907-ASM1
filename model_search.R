...
search_best_model <- function(dataset, config, target_col = "target_21d", candidate_predictors = NULL, date_col = "date", ticker_col = "ticker", min_predictors = 1, max_predictors = NULL, compute_ic = TRUE) {
  ...
  combos_by_k <- vapply(k_values, function(k) utils::combn(length(avail), k), numeric(1))
...
get_stage4_k_values <- function(config, min_predictors, max_predictors) {
  k_values <- config$stage4_k_values
  if (is.null(k_values)) k_values <- seq(min_predictors, max_predictors)
  k_values <- sort(unique(as.integer(k_values)))
  k_values <- k_values[k_values >= min_predictors & k_values <= max_predictors]
  k_values
}
